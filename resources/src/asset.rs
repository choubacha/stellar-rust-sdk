...
pub struct Asset {
    ...
    asset_code: String,
    asset_issuer: String,
    #[serde(deserialize_with = "utils::deserialize_from_str")] amount: f64,
    num_accounts: u32,
    flags: Flag,
}
...
impl Asset {
    ...
    pub fn amount(&self) -> i64 {
        (self.amount * 10_000_000.0) as i64
    }
    ...
    /// improperly.
    /// Returns a bool.
    pub fn auth_revocable(&self) -> bool {
...
mod tests {
    ...
    fn it_parses() {
        ...
        assert_eq!(asset.asset_issuer(), "GBAUUA74H4XOQYRSOW2RZUA4QL5PB37U3JS5NE3RTB2ELJVMIF5RLMAG");
        assert_eq!(asset.amount(), 1000000000);
        assert_eq!(asset.num_accounts(), 91547871);
        assert_eq!(asset.auth_required(), false);
        assert_eq!(asset.auth_revocable(), false);
    }
